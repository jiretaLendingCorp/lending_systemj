 ...
 * Verification links with time-limited tokens.
 */
...
const FROM_EMAIL = Deno.env.get("FROM_EMAIL") ?? "noreply@lendflow.ph";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Can be called by system (cron) or by admin
    ...
    switch (email_type) {
      case "email_verification": {
        ...
        subject = "Verify Your Email - LendFlow";
        htmlBody = `
          <!DOCTYPE html>
          <html>
          <head><meta charset="utf-8"></head>
          <body style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #1a56db;">LendFlow Email Verification</h2>
      ...
      case "password_reset": {
        ...
        subject = "Reset Your Password - LendFlow";
        htmlBody = `
          <!DOCTYPE html>
          <html>
          <head><meta charset="utf-8"></head>
          <body style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #1a56db;">LendFlow Password Reset</h2>
      ...
      case "loan_status": {
        ...
        subject = `Loan ${loan_status === "approved" ? "Approved" : "Update"} - LendFlow`;
        htmlBody = `
          <!DOCTYPE html>
          <html>
          <head><meta charset="utf-8"></head>
          <body style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #1a56db;">LendFlow Loan Update</h2>
            <p>Your loan application for ₱${formattedAmount} has been <strong>${loan_status}</strong>.</p>
            <p>Log in to your LendFlow account for more details.</p>
          </body>
          </html>
        `;
        break;
